...
| APPL (1001-6 §6) | **17** | 20 | `test_yellowbook_appl.c` |
| COSEM objects (1001-6 §7) | **7 тестов** | 7 процедурных | `test_yb_ats_al_cosem_objs.c` |
| SYMSEC_0 (1001-6 §8) | **11** | 15 | `test_yb_ats_al_symsec_0.c` |
| E2E интеграционные | **26** | 26 | `test_yellowbook_e2e.c` |
| ATS_DL unit | **34** | 34 | `test_yb_ats_dl_hdlc_*.c` (5 файлов) |

**Не реализованы:**
- APPL_DATA_SN_N1/N2/N3 — SN referencing не поддерживается библиотекой (только LN)
- SYMSEC_0_FraCount_3 — Send frame counter (требует реального крипто-контекста)
- SYMSEC_0_SecDataX_N1 — Запись STA1 при некорректном шифровании
- SYMSEC_0_SecPol_2/3 — Активация политики безопасности (требуют transition-машину)
...
**Итого: 20 тест-кейсов. Реализовано: 17.** Пропуски (OPEN_8, OPEN_10, DATA_LN_N2) — намеренные исключения ATS.

**Не реализовано:** APPL_DATA_SN_N1/N2/N3 — SN referencing не поддерживается.
...
| # | Test case | Название / цель | Группа | Стр. док. | PDF-стр. | Статус |
|---|---|---|---|---|---|---|
...
**Итого: 15 тест-кейсов. Реализовано: 11 из 15.**

Дополнительно реализованы 4 теста, не входящие в Yellow Book (расширенное покрытие):
- `SYMSEC_0_GLO_GET` — GLO-шифрование GET-запроса
- `SYMSEC_0_GLO_SET` — GLO-шифрование SET-запроса
- `SYMSEC_0_IC_monotonic` — Монотонный инкремент invocation counter
- `SYMSEC_0_IC_overflow` — Overflow invocation counter
...
| 1001-6 §6 Application layer (тест-кейсы) | 17 | 20 | `test_yellowbook_appl.c` |
| 1001-6 §7 COSEM objects (процедурные тесты) | 7 | 7 | `test_yb_ats_al_cosem_objs.c` |
| 1001-6 §8 SYMSEC_0 (тест-кейсы) | 11 | 15 | `test_yb_ats_al_symsec_0.c` |
| E2E интеграционные | 26 | 26 | `test_yellowbook_e2e.c` |
| ATS_DL unit (frame/address/ndm2nrm/info/ndmop) | 34 | 34 | `test_yb_ats_dl_hdlc_*.c` |

**31 CTest-таргетов, все проходят.**

**Не реализовано (6 тест-кейсов):**
1. APPL_DATA_SN_N1 — SN referencing не поддерживается
2. APPL_DATA_SN_N2 — SN referencing не поддерживается
3. APPL_DATA_SN_N3 — SN referencing не поддерживается
4. SYMSEC_0_FraCount_3 — требует реального крипто-контекста
5. SYMSEC_0_SecDataX_N1 — требует негативного сценария шифрования
6. SYMSEC_0_SecPol_2/3 — требуют transition-машину политики безопасности
