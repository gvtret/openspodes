...
static osp_err_t pf_get(const void *inst, uint8_t attr_id, osp_value_t *result) {
	...
	switch (attr_id) {
		...
		case 3:
			...
			return OSP_OK;
		case 4: {
			if (f->capture_objects.count == 0) {
				*result = osp_ic_val_empty_array();
				return OSP_OK;
			}
			result->tag = OSP_TAG_CAPTURE_OBJECT_LIST_REF;
			result->as.ref = &f->capture_objects;
			return OSP_OK;
		}
		default:
...
static osp_err_t pf_set(void *inst, uint8_t attr_id, const osp_value_t *value) {
	...
	switch (attr_id) {
		...
		case 4: {
			...
			f->capture_objects.count = 0;
			for (uint8_t i = 0; i < value->as.array.elements.count && i < OSP_MAX_CAPTURE_OBJECTS; i++) {
				if (osp_ic_read_capture_object(&value->as.array.elements.items[i],
				                               &f->capture_objects.items[f->capture_objects.count]) != OSP_OK) {
					return OSP_ERR_INVALID;
				}
				f->capture_objects.count++;
			}
			return OSP_OK;
		}
		default:
