...
{
    if (!buf || !tag || osp_buf_unread(buf) == 0) return OSP_ERR_INVALID;
    ...
    if (tag->tag_number == 0x1F) {
        ...
        uint8_t byte;
        do {
            if (osp_buf_unread(buf) == 0) return OSP_ERR_INVALID;
            byte = buf->buf[buf->rd++];
...
{
    if (!buf || !len || osp_buf_unread(buf) == 0) return OSP_ERR_INVALID;
    ...
    if (first < 0x80) {
        ...
        *len = buf->buf[buf->rd++];
        ...
        if (osp_buf_unread(buf) < 2) return OSP_ERR_INVALID;
...
{
    if (!buf || !val || osp_buf_unread(buf) == 0) return OSP_ERR_INVALID;
...
osp_err_t osp_ber_read_octet_string(osp_buf_t *buf, uint8_t *out, uint32_t max_len, uint32_t *out_len)
{
    ...
    if (tag.tag_number != 4) return OSP_ERR_INVALID; /* not OCTET STRING */

    uint32_t len;
    r = osp_ber_read_length(buf, &len);
    if (r != OSP_OK) return r;
    if (len > max_len) return OSP_ERR_NOMEM;
...
{
    if (!buf) return OSP_ERR_INVALID;

    uint8_t first = ((tag_class & 0x03) << 6) | ((constructed ? 1 : 0) << 5);

    if (tag_number < 0x1F) {
        if (osp_buf_free(buf) < 1) return OSP_ERR_NOMEM;
        buf->buf[buf->wr++] = first | tag_number;
    } else {
        /* Long tag: need 2+ bytes */
        if (osp_buf_free(buf) < 2) return OSP_ERR_NOMEM;
...
osp_err_t osp_ber_write_uint(osp_buf_t *buf, uint32_t val)
{
    ...
    if (val < 0x80) {
        ...
        if (osp_buf_free(buf) < 5) return OSP_ERR_NOMEM;
...
{
    if (!buf || !tag || osp_buf_unread(buf) < 1) return OSP_ERR_INVALID;
    *tag = buf->buf[buf->rd++];
    return OSP_OK;
}
...
{
    if (!buf || !val || osp_buf_unread(buf) < 1) return OSP_ERR_INVALID;
    *val = buf->buf[buf->rd++];
    return OSP_OK;
}
...
{
    if (!buf || !val || osp_buf_unread(buf) < 2) return OSP_ERR_INVALID;
...
{
    if (!buf || !val || osp_buf_unread(buf) < 4) return OSP_ERR_INVALID;
    *val = ((uint32_t)buf->buf[buf->rd] << 24) |
...
osp_err_t osp_axdr_read_octet_string(osp_buf_t *buf, uint8_t *out, uint32_t max_len, uint32_t *out_len)
{
    ...
    if (r != OSP_OK) return r;
    if (tag != OSP_AXDR_OCTETSTRING) return OSP_ERR_INVALID;
...
{
    if (!buf || osp_buf_free(buf) < 1) return OSP_ERR_INVALID;
    buf->buf[buf->wr++] = tag;
    return OSP_OK;
}
...
{
    if (!buf || osp_buf_free(buf) < 2) return OSP_ERR_INVALID;
    buf->buf[buf->wr++] = (uint8_t)(val >> 8);
    buf->buf[buf->wr++] = (uint8_t)(val & 0xFF);
    return OSP_OK;
}
...
{
    if (!buf || osp_buf_free(buf) < 4) return OSP_ERR_INVALID;
    buf->buf[buf->wr++] = (uint8_t)(val >> 24);
    buf->buf[buf->wr++] = (uint8_t)(val >> 16);
    buf->buf[buf->wr++] = (uint8_t)(val >> 8);
    buf->buf[buf->wr++] = (uint8_t)(val & 0xFF);
