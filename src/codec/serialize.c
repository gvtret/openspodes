...
osp_err_t osp_axdr_read_i32(osp_buf_t *buf, int32_t *val)
{
    ...
    if (r == OSP_OK) *val = (int32_t)v;
    return r;
}

osp_err_t osp_axdr_read_i64(osp_buf_t *buf, int64_t *val)
{
    if (!buf || !val || osp_buf_unread(buf) < 8) return OSP_ERR_INVALID;
    *val = ((int64_t)buf->buf[buf->rd] << 56) | ((int64_t)buf->buf[buf->rd+1] << 48) |
           ((int64_t)buf->buf[buf->rd+2] << 40) | ((int64_t)buf->buf[buf->rd+3] << 32) |
           ((int64_t)buf->buf[buf->rd+4] << 24) | ((int64_t)buf->buf[buf->rd+5] << 16) |
...
{
    if (!buf || !val || osp_buf_unread(buf) < 8) return OSP_ERR_INVALID;
    *val = ((uint64_t)buf->buf[buf->rd] << 56) | ((uint64_t)buf->buf[buf->rd+1] << 48) |
           ((uint64_t)buf->buf[buf->rd+2] << 40) | ((uint64_t)buf->buf[buf->rd+3] << 32) |
           ((uint64_t)buf->buf[buf->rd+4] << 24) | ((uint64_t)buf->buf[buf->rd+5] << 16) |
...
{
    if (!buf || osp_buf_free(buf) < 8) return OSP_ERR_INVALID;
...
osp_err_t osp_axdr_read_visible_string(osp_buf_t *buf, char *out, uint32_t max, uint32_t *len)
{
    ...
    if (tag != 10) return OSP_ERR_INVALID; /* visible-string */

    uint32_t slen;
    r = osp_axdr_read_u32(buf, &slen);
    if (r != OSP_OK) return r;
    if (slen > max - 1) return OSP_ERR_NOMEM;
...
osp_err_t osp_axdr_write_visible_string(osp_buf_t *buf, const char *str, uint32_t len)
{
    ...
    if (r != OSP_OK) return r;
    if (osp_buf_free(buf) < len) return OSP_ERR_NOMEM;
...
{
    if (!buf || !date || osp_buf_unread(buf) < 5) return OSP_ERR_INVALID;
...
{
    if (!buf || !date || osp_buf_free(buf) < 5) return OSP_ERR_INVALID;
...
{
    if (!buf || !time || osp_buf_unread(buf) < 4) return OSP_ERR_INVALID;
...
{
    if (!buf || !time || osp_buf_free(buf) < 4) return OSP_ERR_INVALID;
...
osp_err_t osp_datetime_read(osp_buf_t *buf, osp_datetime_t *dt)
{
    ...
    if (osp_buf_unread(buf) < 3) return OSP_ERR_INVALID;
    buf->rd += 3;
    return OSP_OK;
}
...
osp_err_t osp_datetime_write(osp_buf_t *buf, const osp_datetime_t *dt)
{
    ...
    if (r != OSP_OK) return r;
    if (osp_buf_free(buf) < 3) return OSP_ERR_NOMEM;
...
osp_err_t osp_struct_begin_read(osp_buf_t *buf, uint8_t *num_fields)
{
    ...
    osp_err_t r = osp_axdr_read_tag(buf, &tag);
    if (r != OSP_OK) return r;
    if (tag != OSP_AXDR_STRUCTURE) return OSP_ERR_INVALID;
...
osp_err_t osp_array_begin_read(osp_buf_t *buf, uint8_t *count)
{
    ...
    osp_err_t r = osp_axdr_read_tag(buf, &tag);
    if (r != OSP_OK) return r;
    if (tag != OSP_AXDR_ARRAY) return OSP_ERR_INVALID;
...
osp_err_t osp_bitstring_read(osp_buf_t *buf, uint8_t *bits, uint32_t max_bits, uint32_t *num_bits)
{
    ...
    osp_err_t r = osp_axdr_read_tag(buf, &tag);
    if (r != OSP_OK) return r;
    if (tag != OSP_AXDR_BITSTRING) return OSP_ERR_INVALID;

    uint32_t len;
    r = osp_axdr_read_u32(buf, &len);
    if (r != OSP_OK) return r;
    if (len == 0) { *num_bits = 0; return OSP_OK; }
    if (osp_buf_unread(buf) < len) return OSP_ERR_INVALID;

    uint32_t total_bits = len * 8;
    /* Last byte may have unused bits (specified by first byte of bitstring) */
    if (total_bits > max_bits) return OSP_ERR_NOMEM;
...
{
    if (!buf || !val) return OSP_ERR_INVALID;
    ...
    case OSP_TAG_OCTETSTRING: {
        ...
        r = osp_axdr_read_u32(buf, &slen);
        if (r != OSP_OK) return r;
        if (slen > OSP_MAX_OCTET_LEN) return OSP_ERR_NOMEM;
        if (osp_buf_unread(buf) < slen) return OSP_ERR_INVALID;
...
osp_err_t osp_value_write(osp_buf_t *buf, const osp_value_t *val)
{
    ...
    case OSP_TAG_OCTETSTRING:
        return osp_axdr_write_octet_string(buf, val->as.octetstring.data,
                                            val->as.octetstring.len);

    case OSP_TAG_VISIBLESTRING:
        return osp_axdr_write_visible_string(buf, val->as.visiblestring.data,
                                             val->as.visiblestring.len);
...
{
    if (!buf || !obis) return OSP_ERR_INVALID;
    /* OBIS encoded as 6 octets (no tag, no length — inline) */
    if (osp_buf_unread(buf) < 6) return OSP_ERR_INVALID;
...
{
    if (!buf || !obis || osp_buf_free(buf) < 6) return OSP_ERR_INVALID;
...
osp_err_t osp_access_right_read(osp_buf_t *buf, osp_access_right_t *ar)
{
    ...
    for (uint8_t i = 0; i < acount; i++) {
        ...
        uint8_t am; osp_axdr_read_u8(buf, &am);
    ...
    if (r != OSP_OK) return r;
    if (mcount > OSP_MAX_METHOD_ITEMS) mcount = OSP_MAX_METHOD_ITEMS;
    ...
    for (uint8_t i = 0; i < mcount; i++) {
        ...
        uint8_t mm; osp_axdr_read_u8(buf, &mm);
